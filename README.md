...
- **Quotes browser look** borrows the Inx export-card presentation: a bold book title, an `by <author>` byline (from the book's cached metadata), a `Chapter N · p. M` location label (from the ANN3 relocation), a divider, and the quote itself in a **content-hugging outlined card with a left ink accent bar** (a nod to the highlighter stroke in Inx's export cards).
