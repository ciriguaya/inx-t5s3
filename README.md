...
- Deleting a highlight removes it from **every** store, wherever the delete happens: the in-book long-press popup removes the per-page ANN3 shard record **and** the `/highlights` JSON + `*_pages.json` master; the **Quotes browser delete does the same** (it sweeps the quote out of the book's shards by text, so the highlight disappears from the book too).
- **The ANN3 shards are purely derived from `/highlights`** — there is a single source of truth, not two parallel systems to keep in sync. Every book open reconciles the shards against the quote list (any shard record whose text is no longer in `/highlights` — deleted via the browser, a failed save, or drift — is dropped), so a stale record can never resurrect a deleted highlight.
...
- **Quotes browser look** borrows Inx's **book-page highlight** card: a bold book title, an `by <author>` byline (from the book's cached metadata), and the quote rendered **in the context of its book page** — the cached page text is word-wrapped at 12 pt onto a page-styled card with a margin line, and the quoted span is marked with the same lattice highlight band used in-book. When the page isn't cached (or the quote can't be re-located), it falls back to the stored paragraph, then the quote itself. A `Chapter N · p. M` footer (from the ANN3 relocation) sits at the bottom of the page card. Words are wrapped by measured width, so no line is ever clipped.
...
### Highlights & annotations in the WiFi export (compatible with Inx's export page)
- The web **Bookmarks & Annotations export** (Manage via WiFi → `/export`) previously only saw Inx's native stores (bookmarks + ANN3 annotation shards), so quotes from the custom `/highlights` system — especially fork quotes the reader never relocated — were missing. The export index builder now **merges every `/highlights/*.json` + `*_pages.json` quote** as an annotation entry (book title/author/cover resolved from the book's cache when available, otherwise from the quote itself), deduped against the ANN3 entries by normalized book + text so nothing doubles.
- The index **rebuilds automatically when `/highlights` changes** (a signature of the directory is stored in the index header and compared on every request), so quotes added by the myT5S3-Reader fork while this firmware was off appear without a manual refresh.
- The index build is **thread-safe** (a mutex serializes concurrent rebuilds, and the recents list is snapshotted instead of re-loaded per entry) — two browsers hitting `/api/export-notes` at once can no longer tear the JSON.
