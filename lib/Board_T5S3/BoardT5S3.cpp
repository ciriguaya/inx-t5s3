...
// 8-bit / 5000 Hz is the config this hardware was shipped with and it is the only safe resolution here:
// at 13 bits the 80 MHz APB clock needs a timer pre-divider of ~1.95, which the S3's LEDC cannot
// represent (ledc_timer_config fails with ESP_FAIL "no proper pre-divider number"), so the channel
// never outputs and the backlight stays dead. With 8 bits the divider is 62.5 - well inside range.
...
uint8_t backlightDutyForLevel(uint8_t level) {
  ...
  // Level 1 uses duty 2/255 (0.8%): the dimmest level that still visibly lights this LED (duty 1/255
  // was below the switch/LED's conduction threshold and looked off), and still ~1.5x dimmer than the
  // original curve's level 1 (1.2%). Levels 2-10 keep the original quadratic curve untouched.
  if (level == 1) {
    return 2;
  }
